...
- Primary theme color (`primary`) and shadows MUST be dynamic via `context.primaryColor` and `context.primaryShadow` (`color_utils.dart`); never hardcode `AppColors.primary` in UI widgets/navbars.
