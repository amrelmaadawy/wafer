...
class MockOwnerDashboardRemoteDataSource extends Mock
...
void main() {
  ...
  final tDashboardModel = OwnerDashboardModel.fromJson(
    const {},
  ); // Empty or mock data
  ...
  group('getDashboardStats', () {
    test(
      ...
      () async {
        // arrange
        when(
          () => mockRemoteDataSource.getDashboardStats(
        ...
        // assert
        verify(
          () => mockRemoteDataSource.getDashboardStats(forceRefresh: false),
        );
    ...
    test(
      'should retry with forceRefresh=true if cache deserialization fails with TypeError',
      () async {
        // arrange
        when(
          () => mockRemoteDataSource.getDashboardStats(forceRefresh: false),
        ...
        when(
          () => mockRemoteDataSource.getDashboardStats(forceRefresh: true),
        ).thenAnswer((_) async => tDashboardModel);
        // act
    ...
    test(
      'should return ServerFailure if both cache and forceRefresh network calls fail with type error',
      () async {
        // arrange
        when(
          () => mockRemoteDataSource.getDashboardStats(
